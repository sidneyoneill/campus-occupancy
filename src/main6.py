...
CONFIG = {
    ...
    "output_path": "output/combined_occupancy_main6_v8.MOV",
    ...
    "json_output_path": "output/detection_results_main6_v8.json",  # New path for JSON output
    ...
    "chair_confidence": 0.01,         # Confidence threshold for chair detection
    "chair_object_threshold": 0.5,   # If chair detection is below this, may indicate a bag on the chair
    "show_chair_confidence": True,   # Whether to display chair detection confidence in visualization
    ...
    while True:
        ...
        if cycle_index < person_cycle_length:
            ...
            for roi_id, chair in chair_tracker.items():
                ...
                # Check if a person is detected on this chair
                ...
                # If no person detected, check if chair detection confidence is below the object threshold
                # which might indicate an object like a bag on the chair
                if not person_detected and chair["confidence"] < CONFIG["chair_object_threshold"]:
                    debug_print(f"Frame {frame_count}: Chair {roi_id} has low detection confidence ({chair['confidence']:.2f}), possible object on chair")
                    occupancy_evidence[roi_id]['positive'] += 1
        ...
        for roi_id, chair in chair_tracker.items():
            ...
            # Display text with or without confidence based on config
            if CONFIG.get("show_chair_confidence", False):
                # Add confidence to the status text
                conf_text = f"Chair [{roi_id}]: {status} (conf: {chair['confidence']:.2f})"
                cv2.putText(vis_frame, conf_text, (pt1[0], pt1[1] - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            else:
                # Original display without confidence
