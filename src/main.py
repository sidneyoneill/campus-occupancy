...
class ChairDetector:
    ...
    """
    def __init__(self, model="yolov8m_chair_cpu", confidence=0.4):
...
def process_video():
    ...
    output_path = "output/processed_video.MOV"
    roi_path = "annotations/chair_locations_2.json"
    keyframe_interval = 30   # Do new chair detection every 30 frames
    ...
    # The chair tracker maps an ROI id to the latest detection: { "bbox": [...], "confidence": ..., "occupied": bool }
    chair_tracker = {}
    ...
    person_detector = PersonDetector(model="yolov8s", confidence=0.3)
    chair_detector = ChairDetector(model="yolov8m_chair_cpu", confidence=0.4)
    ...
    while True:
        ...
        # Resize frame for faster detection processing
                ...
                for roi_id, roi_bbox in chair_rois.items():
                    if point_in_bbox(center, roi_bbox):
                        ...
                        chair_tracker[roi_id] = {"bbox": scaled_bbox,
                                                 ...
                                                 "occupied": False}
                        # If a detection falls in an ROI, we update and break out of the loop.
                        break
        ...
        for roi_id, chair in chair_tracker.items():
            ...
            # Change color based on occupancy status
            color = (0, 0, 255) if chair["occupied"] else (0, 255, 0)  # Red if occupied, green if empty
            ...
            cv2.putText(vis_frame, f"Chair [{roi_id}]: {status}", (pt1[0], pt1[1] - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)  # Also update text color

        # Optionally, draw person detections (shown here in yellow)
        for person in person_detections:
            bbox = person["bbox"]
            pt1 = (int(bbox[0]), int(bbox[1]))
            pt2 = (int(bbox[2]), int(bbox[3]))
